...
class SLICProcessor(object):
    ...
    def assignment(self):
        for cluster in self.clusters:
#NOTE self.S distance between 2 clusters
            for h in range(cluster.h - 2 * self.S, cluster.h + 2 * self.S):
                ...
                for w in range(cluster.w - 2 * self.S, cluster.w + 2 * self.S):
                    ...
                    if D < self.dis[h][w]:
                        ...
                        self.dis[h][w] = D
        # logging distance
            wandb.log({"Average Distance":np.mean(self.dis)})
                    


    def update_cluster(self):
        for cluster in self.clusters:
            sum_h = sum_w = sum_L = sum_A = sum_B = number = 0
    ...
    def iterate_10times(self):
        ...
        for i in trange(10):
            ...
            wandb.log({
                ...
                'Clusters':self.K,
                'M':self.M,
                'Image':[wandb.Image(name, caption=f'Iteration {i}')],
...
if __name__ == '__main__':
    ...
    #set run id
    run_id='002'
    parameters = [(200,40),(300,40),(500,40),(1000,40),(200,5),(300,5),(500,5),(1000,5)]
    for K,M in parameters:
        run = wandb.init(
         project='slic-sengmentation',
         config = {'K':K,'M':M},
         #name = f'params:{K}{M}_{run_id}',
         #id = f'{K}'
         )
        p=SLICProcessor('Lenna.png',K,M)
       #wandb.log({'K':K,
        #'M':M,
        #})
        p.iterate_10times()
